...
# 2. System update and basic packages installation
...
check_error "Basic packages installation"

# 3. Hestia CP installation
echo -e "${YELLOW}=== Installing Hestia CP ===${NC}"
{
    echo -e "${BLUE}[Info] Downloading installer script...${NC}"
    wget https://raw.githubusercontent.com/hestiacp/hestiacp/release/install/hst-install.sh
    
    echo -e "${BLUE}[Info] Starting installation (this may take several minutes)...${NC}"
    bash hst-install.sh --lang 'ru' --hostname 'hostname' --username 'TrafficAdmin' --email 'info@domain.tld' --password 'AdMiNiStRaToR' --apache no --named no --exim no --dovecot no --clamav no --spamassassin no --force
    
    echo -e "${BLUE}[Info] Checking service status...${NC}"
    if systemctl is-active --quiet hestia; then
        echo -e "${GREEN}Hestia service is running${NC}"
    else
        ...
        if systemctl is-active --quiet hestia; then
            ...
            echo -e "${RED}Service startup error${NC}"
...
check_error "Hestia CP installation"
...
check_error "Fail2ban configuration"
...
check_error "Grafana installation"
...
check_error "Prometheus installation"
...
check_error "Node Exporter installation"

# 9. Pushgateway installation
echo -e "${YELLOW}=== Installing Pushgateway ===${NC}"
{
...
check_error "Pushgateway installation"

# 10. Loki and Promtail installation
...
check_error "Loki and Promtail installation"

# 11. Fail2ban monitoring setup
echo -e "${YELLOW}=== Setting up fail2ban monitoring ===${NC}"
{
...
check_error "Fail2ban monitoring setup"
...
check_error "Grafana configuration"
...
echo -e "Hestia CP:  admin / (password from file /usr/local/hestia/data/users/admin/password.conf)"
echo -e "Grafana:    admin / admin"
echo -e "\n${RED}IMPORTANT: Change all passwords${NC}"