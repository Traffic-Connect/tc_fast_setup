...
install_base_packages() {
    ...
    # Запускаем установку базовой системы
    install_base_system
    
    # Всегда возвращаем успех, даже если некоторые пакеты не установились
    log_ok "Базовые пакеты установлены (некоторые могут быть пропущены)"
    return 0
}
...
main_installation() {
    ...
    # Установка административной панели ПЕРВОЙ (требует чистый сервер)
    if ! install_admin_panel; then
    ...
    if ! install_base_packages; then
        log_warn "Предупреждение: не удалось установить некоторые базовые пакеты"
        log_info "Продолжаем установку других компонентов..."
    fi
