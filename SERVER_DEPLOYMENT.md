...
chmod +x install_monitoring_only.sh

# Запускаем установку
sudo bash install.sh
```

### 2. Если HestiaCP не устанавливается из-за конфликтов
```bash
# Если установка HestiaCP не удалась, используйте альтернативный скрипт
sudo bash install_monitoring_only.sh
```
...
### 3. Решение проблем с HestiaCP
**Проблема:** HestiaCP требует чистый сервер, но наш скрипт безопасности устанавливает ufw и nginx

**Решение:**
1. Установщик автоматически удаляет конфликтующие пакеты
2. После установки HestiaCP восстанавливает настройки безопасности
3. Если это не помогает, используйте `install_monitoring_only.sh`

**Альтернатива:** Установка только системы мониторинга без HestiaCP
